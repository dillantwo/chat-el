...
export async function POST(req: Request) {
  try {
    ...
    const makeUserText = (p: string) => {
      if (!currentHtml) {
        return `請根據以下要求生成一個可直接執行的數學圖解（單檔 HTML）：${p}`;
      }
      if (targetedEdit) {
        const dynamicNote = targetIsDynamic
          ? `\n\n重要：老師實際點擊的是${targetLabel ? `「${targetLabel}」` : "一個元素"}，但這個節點在原始 HTML 中並不存在——它是由本圖解的 JavaScript 在執行時動態生成的。data-ai-target 標記的只是它最接近的靜態容器。請找出負責生成它的 JavaScript（迴圈、模板或 render 函式）並在那裏修改，容器本身在 HTML 中要維持原本的空白狀態。`
          : "";
...
A. 先讀懂題目在問什麼，答案絕對不能出現在圖解裏。
   ...
   - 圖形一定要畫得出來、看得見：要隱藏的只是「數字」，不是「圖形」。即使某根棒子的高度、某段長度正好等於答案，也**必須照真實比例確實畫出來**，只是旁邊不寫那個數字而已。嚴禁因為怕洩漏答案就把高度／長度設成 0、留空、或乾脆不設值——那會生出一張空白圖，比寫出答案更糟糕。
...
B. 老師說「不要互動」或只要一張「圖」時，就做純靜態的圖。
   ...
   - 只有當老師確實要求可操作、可練習、可輸入、可調整參數時，才做可互動的圖解。
...
3. 介面要清晰、現代、適合桌面與平板。
4. 若老師要的是可互動的圖解，就要真的可互動，不能只是一頁靜態說明；但若老師只要一張圖（見最高優先規則 B），則做純靜態圖，不要硬加互動。
5. 內容以繁體中文呈現。
6. 若題意不足，補上最合理的預設值，但仍要讓圖解可運作。
7. 不要輸出 Markdown code fences。
8. 如果提供了目前 HTML，代表這次是修改既有圖解，不要無故重做成完全不同的圖解；優先保留原本可用的結構，再按要求調整。
8b. 若 HTML 中有某個元素帶有 data-ai-target 屬性，代表老師只想修改「那一個元素」相關的部分，請把改動集中在它身上，其餘內容保持原樣；並且務必在最終輸出中移除 data-ai-target 這個臨時屬性。
8c. 嚴禁內容重複：每一批重複性元素（氣球、方格、卡片、圓點等）只能有單一來源——要麼全部寫死在靜態 HTML 裏、腳本不再生成，要麼容器留空、全部由 JavaScript 生成。兩者不可並存，否則載入後數量會變成兩倍。修改既有圖解時，如果原本是由 JavaScript 生成，就只改生成的程式碼（迴圈次數、資料陣列），不要把生成結果另外寫成靜態 HTML；同理，生成函式必須先清空容器（例如 container.innerHTML = ''）再填入，並確保它不會被重複呼叫而累加。

版面與自適應要求（這個圖解會被嵌入 iframe，請務必遵守）：
9. html、body 設為 height:100%、margin:0，並用 box-sizing:border-box；最外層容器用 min-height:100vh，讓內容能填滿 iframe，全螢幕時也能正常撐開、置中。注意：min-height 不會讓高度變成「確定值」，所以任何需要百分比高度的子元素都不能只靠祖先的 min-height（詳見第 14 條）。
...
11. 內容要能自適應不同尺寸：使用相對單位（%、rem、clamp()、min()、max()）與 flex-wrap，避免固定寬高造成在小視窗溢出或在全螢幕時出現大片空白。文字過長時要能換行，容器要 overflow 安全。「避免固定高度」是指不要寫死一個死板的 px 版面，不是叫你不要給繪圖區高度——繪圖區請用 clamp() 或 vh 這種既自適應、又是確定值的高度。
...
14. 棒形圖／長條圖／比例條（最容易出錯的地方，請逐條檢查）：
    - 繪圖區（棒子所在的那個容器）**必須有明確的 height**，例如 height: clamp(200px, 45vh, 420px)。只寫 min-height 是不夠的：CSS 的百分比高度只有在父容器高度是「確定值」時才生效，父容器 height 為 auto 或只有 min-height 時，子元素的 height:60% 會退回 auto，棒子高度直接變 0，畫面上就只剩一條貼在座標軸上、幾乎看不見的細線。
    - flex-grow 比例同樣需要父容器有明確高度才分配得出來，所以「換成 flex」並不能繞過上一條；不管用百分比、flex 還是 grid，繪圖區都必須先有明確 height。最保險的做法是在 JavaScript 裏讀出繪圖區的實際像素高度，再把每根棒子的高度直接設成 px。
    - 每根棒子都要有可見的最小尺寸（例如 min-height: 6px）和明確的背景色，數值很小時也要看得到。
    - 棒子由 JavaScript 生成時，要確認決定高度的變數真的有值；不可以產生 height: NaNpx、height: undefinedpx、height: px 或 height: 0 這種結果。
    - 輸出前自我檢查一次：每一根棒子都必須在畫面上明顯可見、有實際高度，且彼此比例正確。只要有一根算出來是 0 或看不見，這份輸出就不合格，必須改成明確高度後再輸出。
