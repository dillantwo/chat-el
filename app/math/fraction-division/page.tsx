...
.fa38-root .lang-btn.btn-restart{ padding:5px 12px; font-size:0.85rem; border-color:#e67e22; color:#e67e22; box-shadow:0 3px 0 #e67e22; }
.fa38-root .lang-btn.btn-restart:hover{ background:#e67e22; color:#fff; }
.fa38-root .lang-btn.btn-restart:active{ box-shadow:0 0 0 #e67e22; transform:translateY(3px); background:#d35400; color:#fff; }
...
.fa38-root .bars-column::-webkit-scrollbar{ height:10px; }
.fa38-root .bars-column::-webkit-scrollbar-track{ background:#eef1f3; border-radius:5px; }
.fa38-root .bars-column::-webkit-scrollbar-thumb{ background:#bdc3c7; border-radius:5px; }
.fa38-root .bars-column::-webkit-scrollbar-thumb:hover{ background:#95a5a6; }
.fa38-root .bar-wrap-container{ width:100%; display:flex; flex-wrap:nowrap; gap:15px; justify-content:flex-start; align-items:center;
...
.fa38-root .bar-wrap-container.continuous .bar-unit:not(:first-child){ border-left:none; }
...
.fa38-root .nl-wrap-container{ width:100%; display:flex; flex-wrap:nowrap; justify-content:flex-start; align-items:flex-start;
...
.fa38-root .nl-unit{ position:relative; height:56px; width:calc((100% - (var(--max-wholes) - 1) * 15px) / var(--max-wholes)); min-width:var(--unit-min-w, 120px); flex:none; box-sizing:border-box; }
...
.fa38-root .nl-unit .inline-frac span{ padding:1px 2px; }
/* 帶分數刻度（例如 1 1/8）：去掉 .inline-frac 預設的 0 5px 外距，
   整數和分數才會靠在一起，讀起來像一個帶分數而不是兩個數字。 */
.fa38-root .nl-unit .inline-frac{ margin:0; }
...
<div class="container">
  <div class="header">
    <div class="header-left">
      ...
      <button class="lang-btn btn-restart" onclick="window.__FA38.restart()" title="回到這一題的初始狀態，重新開始操作">↺ 重新開始</button>
    </div>
...
export default function FractionDivisionPage() {
  ...
  useEffect(() => {
     ...
     * 之後不論學生怎麼擴分、量測、填答案，甚至按了「隨機出題」，都能回到這一題最初的樣子。
    ...
    function restart() {
      ...
      // 還在飛的紅色色塊會被 re-parent 到 document.body，重建 anim-area 清不掉它們
      // （比照卸載時的清理），另外把投遞佇列與容器佔用旗標歸零。
      ...
      // 由 updateUI() 整個重建 anim-area 並清空答案輸入框與下方填答區。
      toggleWholeNumber();
    }
