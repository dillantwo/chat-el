...
.fa47-root .bars-column::-webkit-scrollbar{ height:10px; }
.fa47-root .bars-column::-webkit-scrollbar-track{ background:#eef1f3; border-radius:5px; }
.fa47-root .bars-column::-webkit-scrollbar-thumb{ background:#bdc3c7; border-radius:5px; }
.fa47-root .bars-column::-webkit-scrollbar-thumb:hover{ background:#95a5a6; }
.fa47-root .bar-wrap-container{ width:100%; display:flex; flex-wrap:nowrap; gap:15px; justify-content:flex-start; align-items:center;
...
.fa47-root .nl-unit{ position:relative; height:56px; width:calc((100% - (var(--max-wholes) - 1) * 15px) / var(--max-wholes)); min-width:var(--unit-min-w, 120px); flex:none; box-sizing:border-box; }
...
.fa47-root .nl-unit .inline-frac span{ padding:1px 2px; }
/* 帶分數刻度（例如 1 1/8）：去掉 .inline-frac 預設的 0 5px 外距，
   整數和分數才會靠在一起，讀起來像一個帶分數而不是兩個數字。 */
.fa47-root .nl-unit .inline-frac{ margin:0; }
